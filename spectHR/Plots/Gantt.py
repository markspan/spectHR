...
def gantt(dataset, labels=True):
    ...
    if hasattr(dataset, 'active_epochs') and isinstance(dataset.active_epochs, dict):
        ...
        visible_epochs = {epoch: visible for epoch,
                          visible in dataset.active_epochs.items() if visible}
    else:
    ...
    # Remove rows with no visible epochs
    ...
    epochs_gantt = epochs_gantt.sort_values(
        by="start", ascending=False).reset_index(drop=True)
    ...
    # Map epoch names to specific colors
    color_dict = dict(zip(epoch_names, colors))
    ...
    # Convert epoch names to title case
    ax.set_yticklabels([name.title() for name in epoch_names])
