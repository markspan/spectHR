...
def poincare(dataset):
    ...
    df = dataset.RTops.dropna(subset=['epoch'])
    ...
    if not all(col in df.columns for col in required_columns):
        raise ValueError(
            "DataFrame must contain 'ibi', 'epoch', and 'time' columns.")
    if df.shape[0] < 2:
        raise ValueError(
            "The DataFrame must have at least two rows for a Poincaré plot.")
    ...
    if not hasattr(dataset, 'active_epochs'):
        dataset.active_epochs = {
            epoch: True for epoch in dataset.unique_epochs}
    ...
    for unique_epoch in dataset.unique_epochs:
        ...
        mask = [
            unique_epoch in sublist if sublist is not None else False for sublist in dataset.RTops.epoch]
    ...
    for epoch in sorted(dataset.unique_epochs):
        ...
        scatter_handles[epoch] = ax.scatter(
            x, y, label=epoch.title(), alpha=0.2)
        ibm = np.mean(x)  # Mean IBI
        # Color of the scatter points
        col = scatter_handles[epoch].get_facecolor()
    ...
    cursor = mplcursors.cursor(
        [scatter for scatter in scatter_handles.values()], highlight=True, hover=False)
    ...
    ax.axline((0, 0), slope=1, color='gray', linestyle='--',
              linewidth=0.7)  # Line of identity
    # Filter scatter handles based on the dict_values
    scatters = [handle for label, handle in scatter_handles.items()
                if dataset.active_epochs[label]]
    ...
    vbox_layout = Layout(display='flex', flex_flow='column',
                         align_items='flex-start', gap='0px')
    ...
    for epoch in sorted(dataset.unique_epochs, key=lambda v: v.upper()):
        ...
        checkbox.observe(update_visibility, names='v_model')
