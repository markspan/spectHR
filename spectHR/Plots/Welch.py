...
def welch_psd(Dataset, interpolate=True, fs=4, logscale=False,  nperseg=256, noverlap=128, interp_kind='linear', window='hamming'):
    ...
    try:
        freqs, psd = welch(ibi_resampled, fs=fs, scaling='density',
                           nfft=2**12, nperseg=nperseg, noverlap=noverlap, window=window)
    except ValueError:
        return
    ...
    def band_power(frequencies, power_spectrum, band):
        ...
        return np.trapz(power_spectrum[idx], frequencies[idx])
    ...
    vlf_psd_ex = np.insert(vlf_psd, 0, np.interp(
        vlf_band[0], freqs, psd))  # Add exact vlf_band[0]
    vlf_psd_ex = np.append(vlf_psd_ex, np.interp(
        lf_band[0], freqs, psd))  # Add exact lf_band[0]

    lf_psd_ex = np.insert(lf_psd, 0, np.interp(
        vlf_band[1], freqs, psd))  # Add exact vlf_band[1]
    lf_psd_ex = np.append(lf_psd_ex, np.interp(
        hf_band[0], freqs, psd))  # Add exact hf_band[0]

    hf_psd_ex = np.insert(hf_psd, 0, np.interp(
        lf_band[1], freqs, psd))  # Add exact lf_band[1]
    hf_psd_ex = np.append(hf_psd_ex, np.interp(
        hf_band[1], freqs, psd))  # Add exact hf_band[1]

    # Interpolate frequencies to ensure exact band boundaries
    # Add exact vlf_band[0]
    ...
    plt.plot(freqs, psd, '-k', alpha=.5, linewidth=.5,
             label=f'PSD Spectrum {titlestring}')
    ...
    if logscale:
        ...
        plt.ylim(bottom=psd.min() * 0.9, top=psd.max()
                 * 1.1)  # Optionally add some margin
