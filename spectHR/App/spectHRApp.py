    ...
    def on_tab_change(change):
        ...
        if tab_index == 1:  # Poincare tab selected
            with poincarePlot:
                ...
                # Display Poincare plot for the dataset
        ...
        if tab_index == 2:  # Descriptives tab selected
            with descriptives:
                ...
                if hasattr(DataSet, 'psd_Values'):
                    ...
                    # Set display precision for DataFrame
                    pd.set_option('display.precision', 8)
                    DataSet.descriptives_Values = pd.merge(
                        DataSet.descriptives_Values, df, on='epoch', how='outer')
                ...
                def save_to_csv(widget, event, data):
                    ...
                    columns = ['id'] + \
                        [col for col in csv_data.columns if col != 'id']
...
"""
                ...
                save_button = v.Btn(
                    children=[
                        # Add a suitable Font Awesome icon
                        v.Icon(left=True, children=["fa-file-csv"]),
                        "Save as CSV"
                    ],
                    class_="ma-2",
                    color="primary",
        ...
        if tab_index == 4:  # Gantt tab selected
            with Gantt:
                ...
                display(cs.gantt(DataSet, labels=True))
