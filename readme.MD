...
- [Step 2, Inspect the Heartrate timeseries (HR Series tab)](#step-2-inspect-the-ibi-timeseries-ibi-series-tab)
...
- [Step 8, Export your results (Results tab)](#step-8-export-your-results-parameters-tab)
...
<details open>
<summary><a href="#prsa-metrics"><strong>PRSA metrics (DC / AC)</strong></a></summary>
</details>
...
**Alternative: Task Force (1996) bands.** Many clinical and general HRV studies use the [Task Force (1996)](#ref-taskforce1996) convention instead. You can switch to it by editing the band edges under **Settings → Edit settings → Frequency Analysis**:
...
**Ready-made presets.** Load any of these via **Settings → Load settings**; they deep-merge onto your current workspace so directory settings and other preferences are preserved.

| Preset file | Convention | Notes |
|---|---|---|
| `presets/carspan.json` | CARSPAN / Groningen defaults | VLF 0.02–0.06, LF 0.07–0.14, HF 0.15–0.40 Hz; CARSPAN spectral method |
| `presets/taskforce.json` | Task Force (1996) | TP 0.0033–0.40, VLF 0.0033–0.04, LF 0.04–0.15, HF 0.15–0.40 Hz; Welch with Hann window |
| `presets/vuams_welch.json` | VU-DAMS 5.0 | TP/LF/HF bands, Welch with quadratic window and Tarvainen detrending to match VU-DAMS output |

**VU-DAMS preset detail.** If you are comparing spectHR output against VU-DAMS 5.0 (VU Ambulatory Monitoring System software), load `presets/vuams_welch.json`. It sets bands and Welch parameters to match what VU-DAMS computes by default:
...
**Detrending (optional).** Slow, non-stationary trends in the IBI series, warm-up, fatigue, posture shifts, leak into the VLF and LF bands and can bias exactly the mid-frequency band that mental-effort research cares about. spectHR offers the smoothness-priors detrending method of [Tarvainen et al. (2002)](#ref-tarvainen2002), which removes such trends without the band-edge distortion of a plain high-pass filter. It is **off by default**; enable it with the **smoothness-priors detrend** checkbox under **Settings → Edit settings → PSD Settings**. When enabled, the adjacent **detrend λ** field sets the regularisation strength (default 500, the value from the original paper; higher values remove slower trends). Detrending conditions the tachogram before PSD computation and applies to all PSD methods (**Welch**, **CARSPAN**, **Lomb-Scargle**); the λ field is greyed out while the toggle is off so it acts as a memory of the last-used value.

**Normalisation.** Absolute spectral power in ms² scales with mean IBI squared, increasing at lower heart rates. The CARSPAN mMI² normalisation corrects for this automatically. For the Welch method the output unit is configurable: the default is **mMI²** (normalised, heart-rate-independent) but can be switched to **ms²** (raw IBI power, not normalised) via the `"units"` key in *Edit settings*. Whether to normalise HRV for mean heart rate is genuinely unresolved ([de Geus et al., 2019](#ref-degeus2019)), and no single recommendation fits all research questions.
...
1. Launch the application. On first run it creates a default settings file, `~/workspace.json`, holding both the analysis parameters and your working directories (defaulting to a folder under Documents).
2. Open **Settings → Directory settings…** to point spectHR at the folder where your data files live, then **Settings → Save settings** (Ctrl+S) to keep it. Settings persist between sessions only when you save them.
...
| **HR Series** | Review the Heartrate timeseries |
...
| **Transfer profile** | Sliding-window time course of the per-band modulus and phase (coherence-gated) |
| **Spectrogram** / **Spectrogram 3D** | Per-epoch time-frequency power as a 2-D heat map or an interactive 3-D surface, with the breathing-frequency trace overlaid |
| **Results** | See all HRV metrics in a table, with an **Export…** button that writes a scalar CSV (the table, opens in JASP / SPSS) and an HDF5 file (full spectra, profile and transfer time series), then optionally the plots |
...
### Step 2: Inspect the Heartrate timeseries (HR Series tab)

Once the peaks look correct, switch to the **HR Series tab** to see heart rate over time in beats per minute. If a breathing signal is available it is shown as a faint green overlay.
...
Ranges are configurable. The shaded grey band is the **confidence interval**. Three PSD methods are available (`carspan`, `carspan_strict`, `welch`), selected in **Settings → Edit settings**. The method takes effect immediately without restarting.
...
Each subplot is one epoch; each trace is one band, coloured to match the band table. The x-axis is time within the epoch (seconds, with `t = 0` at the epoch's first R-peak); the y-axis is band power in mMI² (or ms², if you switched the Welch units). A row of **band checkboxes** at the top selects which bands are drawn (toggling re-draws instantly, without recomputing). The **Equal y-axis** checkbox links every epoch subplot to a common y-max so band-power magnitudes are directly comparable; Up / Down arrows then zoom that axis. A **Grid** checkbox just below it overlays a faint dotted background grid on every tile.

How the curve is computed: a fixed window of length `window_s` slides along the epoch in steps of `step_s`, and the active PSD method (CARSPAN-strict by default) is recomputed on the R-peaks inside each window. The band-power integration that produces one dot of the trace is the **same `band_powers()` call** used in the PSD tab, just applied per-window; flipping the PSD method in *Edit settings* therefore affects the Profiles tab the same way it affects the PSD tab.

Configure it under **Settings → Edit settings → Profile Settings** (see [Spectral profiles](#spectral-profiles) for the full parameter table).
...
An alternative to the profiles plot is the **Spectrogram**, where the x-axis is time in the epoch, the y-axis is frequency, and colour depicts the power at that frequency and time. A **Spectrogram 3D** view shows the same surface in three dimensions (orbit it with the mouse). When a respiration channel is present, the per-window breathing frequency is overlaid on both.
...
The tab is enabled when the recording carries a **blood-pressure or respiration** channel; with neither, the View-menu entry is greyed out and trying to open it pops a message explaining why. The default input is `bp_sys`; on a recording that has respiration but no blood pressure the dock automatically falls back to `rsp` so the view still populates (you can still set `input_signal` explicitly in Transfer Settings).
...
A sibling **Transfer profile tab** shows those summaries as time series. Inside each epoch a fixed window slides at a fixed step (same parameters used by the Profiles tab) and the per-band BandTransfer summary (mean modulus, mean phase, weighted coherence) is collected per window. The tile stacks **two panels sharing the time axis**, mirroring the Bode layout: per-band **modulus** on top (one coloured line per band, with a translucent under-curve fill) and the matching per-band **phase** below (wrapped, y-axis in π). The same **band checkboxes** as the Profiles tab pick which bands are drawn; the **Equal y-axis** checkbox links the modulus scale across epochs and Up / Down arrows zoom it, and a **Grid** checkbox just below it overlays a faint dotted background grid.

Both views share Settings, configurable under **Settings -> Edit settings -> Transfer Settings**:
...
Both transfer views stack panels that share one x-axis. The **Bode tile** (per-epoch view) is three rows (modulus / phase / coherence) plotted against frequency; the **Transfer-profile tile** is two rows (modulus / phase) plotted against time within the epoch.

| Row | Bode tile (per epoch) | Transfer-profile tile (sliding window) |
|---|---|---|
| **Top: modulus** | $\|H(f)\|$, a black line with PSD-style under-curve fills coloured by band. A peak at the breathing frequency means heart rate tracks breathing strongly there. The legend reports the coherence-gated mean $\|H\|$ per band (the same number that lands in `<band>_modulus`). Up / Down arrows shrink / grow the y-axis. | One coloured line per band: how the band-mean modulus changes window by window. The **Equal y-axis** checkbox links the y-top across epochs for cross-epoch reading; Up / Down arrows zoom it. |
...
| **Bottom: coherence** | $\|C(f)\|^2 \in [0, 1]$, the linear predictability of heart rate from breathing per bin. A red dashed line sits at `min_coherence` so the visual gate always matches the integrator's gate. If the recording's peak coherence falls below 0.2 the y-axis auto-rescales (with the peak value annotated on the y-label) so a faint trace stays visible instead of hugging the bottom spine. | *(not drawn in the profile tile)* The per-band weighted coherence is still computed, used to gate the modulus / phase summaries, and written to the export (`<band>_tf_coherence`). |

On the Bode tile (frequency axis), vertical band-coloured shading on the phase and coherence panels marks the configured frequency bands so a peak at, say, 0.25 Hz reads immediately as "inside HF" (the time-axis Transfer-profile panels colour the per-band lines themselves instead). The modulus panel uses PSD-style **under-curve** fills instead of vertical bars so the area visually scales with $\|H\|$ in that band. FullRange is intentionally not drawn on either view: a 0.02 -- 0.5 Hz overlay would blanket the secondary panels, and the integrated $|H|$ over that whole range is rarely meaningful.

All plots, including the Transfer and Transfer-profile tiles, are exported from one place: the **Results** tab's **Export…** button (see [Step 8](#step-8-export-your-results-results-tab)) offers to save the visible docks' figures as PDFs after writing the metrics.

---

### Step 8: Export your results (Results tab)

The **Results tab** shows a table with one row per epoch:

- **Time-domain:** count, mean, median, min, max, RMSSD, SDNN, SDSD, plus `nn50` / `pnn50` and `nn20` / `pnn20` (successive differences over 50 / 20 ms), mean and SD of heart rate (`mean_hr`, `sd_hr`, bpm), the coefficients of variation `cvnn` / `cvsd`, and the geometric `hrv_ti` (triangular index) and `tinn` (triangular interpolation, ms).
- **Poincaré:** SD1, SD2, SD2/SD1, ellipse area, and the Toichi cardiac sympathetic / vagal indices `csi`, `cvi`, `modified_csi`.
- **Non-linear / stationarity:** DFA short-term and long-term scaling exponents (`dfa_a1`, `dfa_a2`) and the reverse-arrangements stationarity z-score (`stationarity_z`), see [Non-linear and stationarity metrics](#non-linear-and-stationarity-metrics) below.
- **PRSA:** Deceleration Capacity (`dc`, ms) and Acceleration Capacity (`ac`, ms) from Phase-Rectified Signal Averaging, see [PRSA metrics](#prsa-metrics) below.
- **Frequency-domain:** One `{band}_power` column per configured frequency band (mMI² by default), with `{band}_pct` (% of total) and `{band}_peak_hz` (peak frequency) per band, plus `total_power`, normalised units `lf_nu` / `hf_nu`, `ln_hf`, and `lf_hf_ratio`. Band columns are data-driven: renaming or adding a band in **PSD Settings** changes the column names automatically. *The LF/HF ratio is reported because the literature expects it, but it should be read descriptively: it is **not** a clean index of sympatho-vagal balance ([Billman, 2013](#ref-billman2013)).*
- **Blood pressure & respiration:** mean SBP, DBP, PP, MAP (`bp_sbp`, `bp_dbp`, `bp_pp`, `bp_map`) and their beat-to-beat variability `sbp_sd` / `dbp_sd`, mean / sample respiratory volume (`resp_mvo`, `resp_svo`), Grossman peak-to-valley respiratory sinus arrhythmia (`rsa`, `rsa0`) ([Grossman et al., 1990](#ref-grossman1990); [Buron & Menuet, 2026](#ref-buron2026)), the mean breathing frequency (`resp_freq`, Hz; `resp_rate_bpm`, breaths/min), respiration-rate variability (`rrv`, s) and an HF-validity flag (`hf_resp_in_band`), see [Respiration-context HF](#respiration-context-hf) below. These columns are always present; the values are blank when the corresponding channel is absent.
- **Transfer (per band):** `{band}_tf_modulus` (coherence-gated mean |H(f)|), `{band}_tf_coherence` (power-weighted mean squared coherence), and `{band}_tf_phase_w` (coherence-gated mean wrapped phase, radians), one set per configured frequency band (excluding FullRange). Present when a blood-pressure or respiration channel is available; blank otherwise. Hover over a column header for a full description.
- **Sympathetic (ICG):** pre-ejection period (`pep`, ms) and the Heather index of contractility (`heather_index`, (dZ/dt)max over the Q-to-C interval) when the recording carries an ICG `dZ/dt` channel, see [Pre-ejection period (PEP)](#pre-ejection-period-pep) below.
- **ECG waveform:** mean T-wave amplitude (`twave_amplitude`), the per-beat height of the T-wave (max of the ECG in `[R+150 ms, R+450 ms]`) above the isoelectric PR-segment baseline, averaged over the epoch (CARSPAN `CalcDataColECGTWAVE`). Upright T-waves give positive values; only genuinely inverted T-waves are negative. **The value is in the ECG channel's own units and is entirely device-dependent** (it scales with electrode placement, gain, and the recorder's A/D calibration), so use it only for *within-recording* comparisons across epochs or conditions; **never compare `twave_amplitude` between recordings made on different devices or montages.**

Each column header carries the metric's docstring as a tooltip.
 ...
 **For children** (typical rate 20–30 bpm) the 1.0 s default could be too large; values of 0.3–0.5 s are more appropriate. Configure `rsa_lag_s` under **Settings → Edit settings → General Settings → Respiration Analysis**.
...
Click the **Export…** button (top-right of the Results tab). It asks for a destination folder, then writes **two files** and offers to export the plots as well:

| File | What it contains |
| --- | --- |
| `{name}.csv` | The metrics table, one row per epoch, every cell a plain scalar (time-domain + Poincaré + integrated band powers + beat-by-beat blood-pressure / respiration / RSA / PEP means + DFA / stationarity). Opens directly in JASP, SPSS, R, or any spreadsheet. |
| `{name}.h5` | HDF5 file with one group per epoch holding **all** the data: the metric scalars *and* the spectral-profile / transfer-function summary scalars as group attributes, plus the full arrays, complete PSD spectra, per-window profile time series, full Bode curves, and sliding-window transfer-function profiles, in sub-groups. |

If you answer **Yes** to "export the plots as well?", a dialog lets you pick a folder and tick which of the open plot tabs to save; each ticked dock's figures are written as PDFs named `{datafile}_{dock}_{epoch}.pdf` (one file per epoch tile for the per-epoch grids).
...
| **PRSA** | `dc` (Deceleration Capacity, ms), `ac` (Acceleration Capacity, ms) |
| **Band powers (PSD)** | `{band}_power` per configured band (mMI² by default), `lf_hf_ratio` |
| **Blood pressure / respiration** | `bp_sbp`, `bp_dbp`, `bp_pp`, `bp_map`, `resp_mvo`, `resp_svo`, `resp_freq`, `hf_resp_in_band`, `rsa`, `rsa0` (always present; blank when the corresponding channel is absent) |
...
/{epoch}/
  attrs: subject,
         ...
         resp_freq, hf_resp_in_band,                           ← respiration-context HF
...
All settings, the analysis parameters **and** the working directories, live in one `~/workspace.json` file, editable through the **Settings** menu.

**Settings → Directory settings…** changes the three folder paths (data, cache, export).

**Settings → Edit settings** is a tabbed dialog:

- **General Settings**: *IBI Classification* (window length, threshold multiplier, TL ceiling), *ECG Preprocessing* (high-pass filter), *Respiration Analysis* (per-epoch flag; RSA lag; RSA overlay; `rsp_source`, `icg` vs `accelerometer`, see [Supported channels](#supported-channels)), *Calibration* (blood-pressure `bp_scale` / `bp_zero`), *ICG Analysis* (PEP B-point guard `b_point_guard_ms`, see [Pre-ejection period](#pre-ejection-period-pep)), *PRSA Analysis* (`prsa_window`, half-window size in beats for DC/AC, default 30, see [PRSA metrics](#prsa-metrics)), *Logging* (minimum log level shown in the Log dock / console).
- **PSD Settings**: PSD method, band edges and colours, method-specific parameters, CI level; **smoothness-priors detrend** toggle and **detrend λ** (default 500; see [Practical considerations](#practical-considerations)).
...
Changes take effect immediately, but are held only in memory until you **Save settings** (Ctrl+S), which writes the whole workspace back to `~/workspace.json`. On the next launch spectHR reloads that file, so settings persist across sessions only once saved. (Window geometry and the dock layout are remembered separately and automatically.)
...
**Default parameters** (all configurable in *Edit settings*):
...
**`hf_resp_in_band`**, a validity flag: `True` when the mean breathing frequency lies inside the configured HF band $[f_\text{HF}^\text{low}, f_\text{HF}^\text{high}]$ (default 0.15–0.40 Hz, i.e. 9–24 breaths/min), `False` when it falls outside, blank when undeterminable. It is the band-membership indicator

$$\mathbb{1}\!\left[\, f_\text{HF}^\text{low} \le f_\text{resp} \le f_\text{HF}^\text{high} \,\right]$$

A `False` is a red flag that the epoch's HF power may not reflect RSA at all. The actual statistical correction (e.g. regressing HF on `resp_freq` across epochs or subjects) is deliberately left to your statistics package (R / JASP), these two columns are the inputs that make it possible.
...
**Phase-Rectified Signal Averaging** (PRSA; [Bauer et al., 2006](#ref-bauer2006)) extracts rhythmic components from a non-stationary IBI series without requiring stationarity. It works by identifying anchor beats, extracting short windows around each anchor, and averaging them: rhythmic patterns that phase-lock to the anchor survive; noise cancels.

**`dc`**, **Deceleration Capacity** (ms). Anchors on beats where the IBI *increased* (the heart slowed). A positive DC reflects the strength of the parasympathetic brake. In post-MI populations, DC is a strong independent predictor of mortality, outperforming SDNN and RMSSD in some cohorts ([Bauer et al., 2006](#ref-bauer2006)).

**`ac`**, **Acceleration Capacity** (ms). Anchors on beats where the IBI *decreased* (the heart sped up). AC is negative by convention; a more negative value indicates stronger acceleration drive (combined sympathetic / vagal withdrawal).

Both are computed by the four-point formula:

$$\text{DC} = \frac{X[0] + X[+1] - X[-1] - X[-2]}{4}, \qquad \text{AC defined analogously}$$

where $X[\cdot]$ is the phase-averaged window (length $2T$) evaluated at the anchor position. Returns NaN when fewer than $2T+2$ clean beats are available or no qualifying anchors exist in the epoch.

The half-window size $T$ (default **30 beats**, the standard clinical value from Bauer et al.) is configurable under **Settings → Edit settings → General Settings → PRSA Analysis → prsa_window**. For short epochs or tachycardic recordings a smaller $T$ (e.g. 10–15) may be necessary.
...
**Default parameters** (configurable in *Edit settings*):
...
Load it with **Settings → Load settings** and point to `presets/vuams_welch.json`. The file is a partial workspace (only the changed keys), so your directory settings and other preferences are preserved by the deep-merge.
...
`carspan_strict` is, by design, the same code as `carspan` with the `carspan_strict_options()` preset applied. If you want to keep `method: "carspan"` in your workspace but flip the algorithm under the hood, **two settings in the Edit settings dialog do the job**:
...
* **Window taper variant.** With `window: "5% cosine bell"` the configurable path uses scipy's Tukey window, which zeros the first/last sample exactly; Index taper gives them a small non-zero weight. The strict preset (`carspan_strict_options()`) hard-codes `taper: "carspan_index"` to get the CARSPAN-faithful version. Difference: ≪ 1 % on band power. Not exposed in the Edit settings dialog because it would be a new "carspan_index" choice for the window dropdown that most users wouldn't recognise; if you need it, set it via the Python API.
* **Mean convention.** `psd_method_from_workspace` sets `mean_convention = "arithmetic"` (arithmetic mean of per-beat rate = harmonic mean of IBI) only when `method == "carspan_strict"`. With `method: "carspan"` you get the manual's `T/N` (harmonic mean of rate). For typical 5–8 % RR variability that's a ~0.3–0.8 % difference in the resulting mMI² values. Not exposed in the Edit settings dialog for the same reason. If you need full bit-equivalence to strict, just use `method: "carspan_strict"`.

The same answer in summary form:

| Goal | What to set in Edit settings |
|---|---|
...
In spectHR this is `FrequencyAnalysis.carspan.dc_removal` (also editable in *Edit settings*), only applicable to the configurable `carspan` variant. It defaults to `false` to preserve historical behaviour; turn it on when you need the VLF cleanup while retaining a custom window or the variance-correct amplitude. The IBI-amplitude `carspan_strict` path does **not** use this mechanism; its mean-subtraction step does the DC removal at the signal level (manual Eq. 3.21).
...
Configure profiles under **Settings → Edit settings → Profile Settings**:
...
**spectHR extension: multiple PSD back-ends.** CARSPAN supports profiles only for the SOC spectral path. spectHR runs the same sliding-window scheme for all four methods (Welch, Lomb-Scargle, CARSPAN, and CARSPAN-strict), dispatching through the same `band_powers()` call used by the PSD tab. The active PSD method in *Edit settings → PSD Settings* applies immediately to profiles.
...
Band edges are configurable in *Edit settings*. If your participants breathe slowly, you may need to extend the HF band to lower frequencies.
...
<a id="ref-bauer2006"></a>
Bauer, A., Kantelhardt, J.W., Barthel, P., Schneider, R., Mäkikallio, T., Ulm, K., Rost, M., Möller, A., Huikuri, H.V., Bunde, A., Schömig, A., & Schmidt, G. (2006). Deceleration capacity of heart rate as a predictor of mortality after myocardial infarction: cohort study. *The Lancet*, 367(9523), 1674–1681. https://doi.org/10.1016/S0140-6736(06)68735-7
